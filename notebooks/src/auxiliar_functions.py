...
class AuxFunctions:
    ...
    @classmethod
    def compute_letters_per_enzyme(cls, df: pd.DataFrame, alpha: float = 0.05, adjust: str = "holm", present_treats=[], treatment_column="Treatment", viability_column="Viability") -> pd.DataFrame:
        """
        ...
        for enz, dsub in df.groupby("Enzyme"):
            ...
            groups = {float(t): dsub.loc[dsub[treatment_column] == t, viability_column].values for t in dsub[treatment_column].cat.categories}
            ...
            for t, L in letters.items():
                rows.append({"Enzyme": enz, treatment_column: t, "Letters": L})

        out = pd.DataFrame(rows)
        if not out.empty:
            out[treatment_column] = pd.Categorical(out[treatment_column], categories=present_treats, ordered=True)
            out = out.sort_values(["Enzyme", treatment_column])
        return out
